...
def telemetry(sid, data):
    if data:
        ...
        try:
            ...
            args.filter.append(steering_angle)
            if len(args.filter) > FILTER_SIZE:
                args.filter = args.filter[-FILTER_SIZE:]            
            # if the speed is above the current speed limit, we are on a downhill.
            # make sure we slow down first and then go back to the original max speed.
            # print(args.filter)
            steering_angle = avg(args.filter)
            global speed_limit
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', help='list of speeds fake arg',  dest='filter',         type=list, default=[])
    # parser.add_argument(
    #     'is_rnn',
    #     type=bool,
    #     nargs='?',
